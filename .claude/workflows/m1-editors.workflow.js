/*
 * m1-editors.workflow.js — Milestone M1 "Editors" (docs/PLAN.md §18-M1).
 * Canvas editor kit (§9: Viewport, layered rendering, gesture FSM), arrangement
 * lanes (clip create/move/trim/split/loop), full piano roll (§10: hit zones,
 * FSM table, keyboard map, snapping, velocity lane), command bus + immer
 * inverse-patch undo (§13), save/load with schemaVersion + OPFS autosave.
 *
 * MODEL POLICY: sonnet = mechanical implementation (persistence plumbing, app
 * shell, polish); opus = load-bearing seams (§9 editor kit, §10 piano-roll FSM,
 * §13 command/undo, arrangement gesture verbs), contracts, integration, ALL
 ...
 *
 * Run standalone:
 *   Workflow({ scriptPath: '/workspaces/fableton/.claude/workflows/m1-editors.workflow.js',
 *              args: { reviewRounds: 3 } })
 ...
 * NEVER call workflow() itself (single nesting level). Assumes M0 is in the tree.
 *
...
export const meta = {
  name: 'daw-m1-editors',
  description: 'M1 Editors: canvas editor kit, arrangement lanes, full piano roll, command bus + inverse-patch undo, save/load + OPFS autosave. Ends in an independent opus acceptance gate.',
  phases: [
    { title: 'Contracts', model: 'opus' },
    { title: 'Implement: kit & state' },
    { title: 'Implement: editors & shell' },
...
let contracts = await agent(
  [
    ...
    SECTION_NOTE,
    'Read ' + PLAN + ' in full, with special care on ' + CRITIC_SECTIONS + '. Then read the existing M0 code under ' + ROOT + '/src to stay consistent with it.',
    'Write/extend shared TypeScript interface files under ' + ROOT + '/src/types/ (you own that directory): editor kit contracts (Viewport per SS9, layer/renderer interfaces, gesture FSM engine API, hit-tester + drag-handler registration), Command/patch/history types (SS13), the v1 Project document shape (channels/clips/notes) and persistence envelope (schemaVersion). Interfaces only — no implementations. Must compile under strict tsc.',
    ...
    'In the schema: interfaceFiles = files you wrote; extraOwnership = optional map pkgId -> additional path prefixes (must not overlap another package; e.g. command-undo may need a grant on an M0 file that must move behind the document); notes = decisions implementers must know, kept short.',
...
if (!contracts) {
  ...
  contracts = { interfaceFiles: [], extraOwnership: {}, notes: 'Contracts agent failed. Implement the TypeScript interfaces exactly as written in PLAN.md SS9/SS10/SS13 and keep them in your own package.' };
} else {
...
const integration = await agent(
  [
    ...
    'Wire the packages into a working whole: mount the arrangement + piano roll in the app shell; route all editor commands through the command bus; connect persistence (autosave + save/load UI); migrate the M0 demo so the scheduler plays clips FROM THE DOCUMENT instead of the hard-coded M0 clip (SS3/SS12 — the document is now the source of truth). Minimal glue only — no redesigns. Cite PLAN sections for spec-relevant wiring decisions.',
...
if (!integration || !integration.tscClean || !integration.testsPassed || !integration.buildOk) {
  ...
  seedFindings.push({ severity: 'blocker', file: '(integration)', description: 'integration incomplete: ' + (integration ? integration.summary : 'integration agent returned nothing'), planSection: 'SS18-M1', lens: 'spec-conformance' });
}
