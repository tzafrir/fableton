...
test("Ctrl+wheel zoom keeps the tick under the cursor fixed", async ({ page }) => {
  ...
  const before = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
  ...
  const after = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
