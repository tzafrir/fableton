...
async function createNoteAt(page: Page, px: number, py: number): Promise<ColorRect> {
  const before = await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL);
  await page.mouse.dblclick(px, py);
  const after = await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL);
...
test.describe("piano roll note editing (SS9/SS10)", () => {
  ...
  test("one gesture equals one undo entry: drag, undo restores exactly, redo reapplies", async ({ page }) => {
    ...
    const moved = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
    ...
    const afterUndo = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
    ...
    const afterRedo = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
    ...
    await page.getByTestId("undo-button").click();
    await page.getByTestId("undo-button").click();
    const afterBothUndo = await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL);
...
test.describe("piano roll keyboard map (SS10)", () => {
  test("arrows transpose/move, Shift+arrows octave/fine, Ctrl+D duplicates, Delete deletes", async ({ page }) => {
    ...
    let r = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
    ...
    r = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
    ...
    const afterDup = await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL);
    ...
    const afterDelete = await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL);
  ...
  test("Ctrl+U quantizes note starts to the grid", async ({ page }) => {
    ...
    // Nudge it 5px off-grid with a fine nudge so it is guaranteed non-grid
    // aligned (12px grid, 5px is not a multiple), then quantize.
    await page.keyboard.press("Shift+ArrowRight");
    await page.keyboard.press("Shift+ArrowRight");
    const offGrid = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
    expect(offGrid.x, "sanity: the note should have moved off its original grid slot").not.toBeCloseTo(note.x, 0);

    await page.keyboard.press("Control+u");
    const quantized = (await scanColorRects(page, "piano-roll-panel", "content", NOTE_FILL))[0] as ColorRect;
